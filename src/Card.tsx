...
import { RedactedNumbers } from "./RedactedNumbers";
...
export function Card({ name, number, expiration, cvv }: CardProps) {
  const [show, setShow] = useState(false);

  const cardRef = useRef<HTMLDivElement>(null);
  return (
    <div
      className="group relative inline-flex items-center justify-center p-4 text-base"
      style={{
        perspective: "1000px",
      }}
      onMouseEnter={() => {
...
export function CardFace({ name, number, expiration, cvv }: CardProps) {
  const [show, setShow] = useState(false);

  return (
    <div className="relative inline-flex items-center justify-center text-base">
      <div
        className="bg-prune-950 border border-stone-100/20 shadow-indigo-900/20"
        style={{
          width: "85.60mm",
          height: "53.98mm",
          borderRadius: "3mm",
